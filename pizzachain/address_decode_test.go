...
import (
	"encoding/hex"
	"github.com/assetsadapterstore/pizzachain-adapter/pizzachain_addrdec"
	"testing"
)

func TestAddressDecoder_AddressEncode(t *testing.T) {
	pizzachain_addrdec.Default.IsTestNet = false

	p2pk, _ := hex.DecodeString("5b1ac00a18dc9bb1a0494206d9ad72ef7fed0873")
	p2pkAddr, _ := pizzachain_addrdec.Default.AddressEncode(p2pk)
	t.Logf("p2pkAddr: %s", p2pkAddr)

	p2sh, _ := hex.DecodeString("adf2c47cbf6a053ebf45b033ba2044c36984a468")
	p2shAddr, _ := pizzachain_addrdec.Default.AddressEncode(p2sh)
...
func TestAddressDecoder_AddressDecode(t *testing.T) {
	...
	p2pkHash, _ := pizzachain_addrdec.Default.AddressDecode(p2pkAddr)
	t.Logf("p2pkHash: %s", hex.EncodeToString(p2pkHash))

	p2shAddr := "Lb5hzBamSSS4xz2FJqAV2cL2bYMq6oDjJA"

	p2shHash, _ := pizzachain_addrdec.Default.AddressDecode(p2shAddr)
